...
public class Ex7_Mart {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 1:
				/* 음료수인지 박스인지 선택
				 * 제품명, 가격, 수량, 박스과자면 1박스당 개수를, 음료수면 용량을
				...
				//올바른 서브메뉴를 선택하면
				case 1: case 2: 
					//제품정보를 입력받아서 판매 제품리스트에 추가
					list[listCount] = creatProduct(subMenu, scan);
					//판매 제품 갯수 증가
					listCount++;
					break;
				//잘못된 서브메뉴를 선택하면
				default : System.out.println("선택할 수 없는 종류입니다.");
				}
				break;
				//선택한 메뉴가 2이면(제품입고)
			case 2:
				...
				//제품을 선택하고 입고할 수량을 선택해서 판매제품 리스트에 입고가 되면
	...
	/* 기능 : 제품 등록에 대한 서브 메뉴를 출력하고, Scanner를 통해서 서브메뉴를 입력받아 서브메뉴를 알려주는 메소드
	 * 매개변수 : Scanner => Scanner scan
	 * 리턴타입 : 입력받은 메뉴 => 정수 => int
	 * 메소드명 : selectSubMenu */
	...
	/* 기능 : 선택한 subMenu에 맞는 제품 정보를 Scanner를 통해 입력받아 제품 객체를 생성한 후 생성한 제품을 
	 * 		 제품을 알려주는 메소드
	 * 매개변수 : 선택한 서브메뉴, 스캐너 => int subMenu, Scanner scan
	 * 리턴타입 : 생성한 제품 => Product
	 * 메소드명 : creatProduct
	 */
	public static Product creatProduct(int subMenu, Scanner scan) {
	...
	public static boolean addAmountProductList(Scanner scan, Product list[], int listCount) {
		...
		//선택된 제품에 입고된 수량을 추가(입고된 수량을 추가)
		list[num-1].sumAmount(amount);
		//입고가 완료됐다고 알려줌
		return true;
	}
	/* 기능 : Scanner를 이용하여 제품과 수량을 선택하면 제품리스트에서 해당 제품을 수량에 맞게 꺼내서 제품을 돌려주는 메소드
	 * 매개변수 : scan, 제품리스트(마트) => Scanner scan, Product list[], int listCount
	 * 리턴타입 : 선택된 제품(입력한 수량으로) => Product (다형성 활용)
	 * 메소드명 : selectProduct*/
