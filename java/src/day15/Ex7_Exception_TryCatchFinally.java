...
public class Ex7_Exception_TryCatchFinally {
	...
	public static void main(String[] args) {
		...
		try {
			 ...
			 *   (예외를 처리할 수 있는 catch가 없는 경우 처리할수 없어서 프로그램 중단) 예외처리를 함.
			 * - catch는 여러개가 올 수 있고(1개 이상), Exception을 처리한다면, Exception은 무조건 마지막 catch에
			 * - catch를 작성할 때 부모 예외들을 자식 아래로 배치.*/
			res = num1 % num2;
			System.out.println(res);
		}
		//RuntimeExceptuon을 ArrayIndexOutOfBoundsException 위에 추가하면 에러발생
