...
public class PageMaker {
	private int totalCount; // 전체 컨텐츠 갯수
	...
	public void calcData() {
		...
		// endPage = (int) (Math.ceil(6 / 10.0 = 0.6 =ceil(올림) = 1 ) * 10) = 10;
		// endPage = (int) (Math.ceil(6/10.0) * 10);
		// endPage = (int) (1.0 * 10) = 10;
		
		startPage = (endPage - displayPageNum)+1;
		// startPage = 1;
		// criteria.getPerPageNum() : 10
		// totalcount : 101
		/* 총 콘텐츠 갯수를 이용하여 마지막 페이지 번호를 계산 */
		// tempEndPage : 마지막 페이지 번호(전체 페이지 네이션중에서)
		...
		prev = startPage == 1 ? false : true; // 첫번째 페이지 네이션이면 이전버튼 비활성
		// prev = criteria.getPage() == 1 ? false : true; // 현재 페이지가 첫번째 페이지이면 이전버튼 비활성
		/* 현재 페이지메이커에 마지막 페이지에 컨텐츠의 마지막이 포함되어 있으면 next가 없어야 함 */
		next = endPage * cri.getPerPageNum() >= totalCount ? false:true; // 마지막 페이지 네이션이면 다음버튼 비활성
		
		// next = criteria.getPage() == endPage ? false : true; // 현재 페이지가 마지막 페이지이면 다음버튼 비활성
	}
